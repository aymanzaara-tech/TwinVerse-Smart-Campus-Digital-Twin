...
import { buildHallStructure } from '../../three/buildHallStructure.js';
import { buildLighting } from '../../three/buildLighting.js';
import { buildFurniture } from '../../three/buildFurniture.js';
import { MeasurementManager } from '../../three/measurementTool.js';
import { createPresentationScreenTexture } from '../../utils/digitaltwin/proceduralTextures.js';
...
export const ThreeCanvas = ({
  ...
  const hallStructRef = useRef(null);
  const hallLightingRef = useRef(null);
  const hallFurnitureRef = useRef(null);
  ...
  // Presentation Screen remains permanently OFF
  useEffect(() => {
    if (hallStructRef.current) {
      hallStructRef.current.screenMaterial.color.setHex(0x0f172a);
      hallStructRef.current.screenMaterial.emissive.setHex(0x000000);
      hallStructRef.current.screenMaterial.emissiveIntensity = 0.0;
      hallStructRef.current.screenMaterial.map = null;
      hallStructRef.current.screenMaterial.emissiveMap = null;
