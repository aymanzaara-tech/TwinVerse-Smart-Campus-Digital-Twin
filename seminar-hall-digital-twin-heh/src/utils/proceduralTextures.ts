...
// 1. Rear Wall Geometric Acoustic Triangles (Matching Photos 2 & 4)
export function createRearAcousticTexture(): THREE.CanvasTexture {
  const canvas = document.createElement('canvas');
  canvas.width = 1024;
  canvas.height = 512;
  const ctx = canvas.getContext('2d')!;

  // Colors based on uploaded photo: deep royal blue, vibrant cyan/teal, slate grey
  const colors = [
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      // Add subtle noise/fabric texture to acoustic felt
...
// 2. Ceiling Acoustic Tile Grid with Geometric Triangles (Matching Photos 1, 2, 4, 5)
export function createCeilingAcousticTexture(): THREE.CanvasTexture {
  const canvas = document.createElement('canvas');
  canvas.width = 1024;
  canvas.height = 1024;
  const ctx = canvas.getContext('2d')!;

  ctx.fillStyle = '#cbd5e1'; // Soft grey base
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      // Random green/blue triangle on some tiles like photo
      if ((r + c) % 3 === 0 || (r * c) % 5 === 0) {
...
// 3. Stage Dark Wood Paneling Texture (Matching Stage Wall in Photo 5)
export function createWoodPanelTexture(): THREE.CanvasTexture {
  ...
  for (let i = 0; i < numPlanks; i++) {
    ...
    // Seams between wood vertical planks
...
export function createFloorTileTexture(): THREE.CanvasTexture {
  ...
  // Rich warm cream / ivory ceramic base
  ctx.fillStyle = '#f3e8d3';
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  // Subtle polished cream marble sheen & tone variation
  for (let i = 0; i < 300; i++) {
  ...
  // Cream / Beige Tile Grout Lines (Large format tile)
  const tileSize = 256;
...
export function createPresentationScreenTexture(slideIndex: number, title?: string): THREE.CanvasTexture {
  const canvas = document.createElement('canvas');
  canvas.width = 1920;
  canvas.height = 1080;
  const ctx = canvas.getContext('2d')!;
...
// 6. Protective Plastic Wrap Sheen Normal / Roughness Map (Matching Plastic Covered Chairs in Photos)
export function createPlasticWrapTexture(): THREE.CanvasTexture {
  const canvas = document.createElement('canvas');
  canvas.width = 512;
  canvas.height = 512;
  const ctx = canvas.getContext('2d')!;
  ...
  // Draw random subtle wrinkle specular lines
...
export function createPodiumEmblemTexture(): THREE.CanvasTexture {
  const canvas = document.createElement('canvas');
  canvas.width = 512;
  canvas.height = 512;
  const ctx = canvas.getContext('2d')!;
  ...
  // Emblem graphics (Flame & Book symbol as seen in Photo 5 on podium)
  ...
  ctx.fillStyle = '#fef08a'; // Inner flame glow
  ctx.beginPath();
...
// 8. Control Room Door Glass with Vertical Text (Matching uploaded photo)
export function createControlRoomDoorTexture(): THREE.CanvasTexture {
  const canvas = document.createElement('canvas');
  canvas.width = 512;
  canvas.height = 1024;
  const ctx = canvas.getContext('2d')!;

  // Light blue/grey translucent glass background
  ...
  // Thin frosted border inset
  ctx.strokeStyle = 'rgba(255, 255, 255, 0.6)';
  ctx.lineWidth = 8;
  ctx.strokeRect(20, 20, canvas.width - 40, canvas.height - 40);

  // Bold white "CONTROL ROOM" text running vertically
  ctx.save();
